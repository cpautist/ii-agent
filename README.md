...
If you want to use OpenRouter (default), set `OPENROUTER_API_KEY` in `.env` and run:
```bash
python cli.py --llm-client openrouter-direct --model-name google/gemini-2.5-flash-preview-05-20
```
...
```bash
export STATIC_FILE_BASE_URL=http://localhost:8000
python ws_server.py --port 8000 --llm-client openrouter-direct --model-name google/gemini-2.5-flash-preview-05-20
```
